...
<div class="flex-1 flex flex-col h-full min-h-0 overflow-hidden">
    <!-- Header -->
    <div
        class="h-10 border-b-2 flex items-center justify-between px-3 shrink-0 shadow-sm z-10 {$isDarkMode
    ...
    >
        ...
        <div class="flex items-center gap-2">
            ...
            <div
                class="w-px h-4 mx-1 {$isDarkMode
                    ? 'bg-slate-700'
                    : 'bg-slate-200'}"
            ></div>
    ...
    >
        ...
        <div
            style="height: {txPanelHeightPercent}%"
            class="flex flex-col min-h-0 overflow-hidden {$isDarkMode
                ? 'bg-[#111113]'
                : 'bg-white'}"
        >
            <SectionHeader
                label={$t("traffic_monitor.outgoing")}
                icon={ArrowUpRight}
        ...
        <div
            ...
            class="h-1 border-y shrink-0 z-20 cursor-row-resize transition-colors {isResizing
                ? 'bg-emerald-500 border-emerald-600'
                : $isDarkMode
                  ? 'bg-black border-slate-800 hover:bg-slate-700'
                  : 'bg-slate-200 border-slate-300 hover:bg-slate-300'}"
        ></div>

        <!-- RX Panel (Takes remaining space) -->
        <div
            class="flex-1 flex flex-col min-h-0 overflow-hidden {$isDarkMode
                ? 'bg-[#09090b]'
                : 'bg-[#fafafa]'}"
        >
            <SectionHeader
                label={$t("traffic_monitor.incoming")}
                icon={ArrowDownLeft}
