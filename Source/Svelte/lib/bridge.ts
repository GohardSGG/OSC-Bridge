...
// --- WebSocket Logic ---
export function initializeBridge(customUrl?: string) {
  ...
  let url = 'ws://localhost:9122/logs';
  if (customUrl) {
    url = customUrl.endsWith('/logs') ? customUrl : `${customUrl}/logs`;
  } else {
    // Try to get from store if no custom URL provided
    let currentWsUrl = 'ws://localhost:9122';
    const unsubscribe = settingsWsUrl.subscribe(value => { currentWsUrl = value; });
    unsubscribe();
    url = `${currentWsUrl}/logs`;
  }
